...
public class CFXEasyEditor : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (basicFoldout)
        {
            ...
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(new GUIContent("Scale Size", "Changes the size of the Particle System(s) and other values accordingly (speed, gravity, etc.)"), GUILayout.Width(120))) applyScale();
            ...
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(new GUIContent("Set Duration", "Changes the duration of the Particle System(s)"), GUILayout.Width(120))) applyDuration();
            ...
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(new GUIContent("Set Delay", "Changes the delay of the Particle System(s)"), GUILayout.Width(120))) applyDelay();
            ...
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(new GUIContent("Loop", "Loop the effect (might not work properly on some effects such as explosions)"), EditorStyles.miniButtonLeft)) loopEffect(true);
            if (GUILayout.Button(new GUIContent("Unloop", "Remove looping from the effect"), EditorStyles.miniButtonRight)) loopEffect(false);
            if (GUILayout.Button(new GUIContent("Prewarm On", "Prewarm the effect (if looped)"), EditorStyles.miniButtonLeft)) prewarmEffect(true);
            if (GUILayout.Button(new GUIContent("Prewarm Off", "Don't prewarm the effect (if looped)"), EditorStyles.miniButtonRight)) prewarmEffect(false);
            GUILayout.EndHorizontal();
        ...
        if (colorFoldout)
        {
            ...
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(new GUIContent("Set Start Color(s)", "Changes the color(s) of the Particle System(s)\nSecond Color is used when Start Color is 'Random Between Two Colors'."), GUILayout.Width(120))) applyColor();
...
//		GUILayout.Space(6);
        ...
        if (copyFoldout)
        {
            ...
            if (GUILayout.Button("Copy properties to selected Object(s)"))
            {
                ...
                if (!foundPs) Debug.LogWarning("Cartoon FX Easy Editor: No Particle System found in the selected GameObject(s)!");
            }
        }
