...
public class FortuneWheelManager : Singleton<FortuneWheelManager>
{
    ...
    private void Awake()
    {
        Debug.Log("Fortune wheel manager is awake!");
        ...
        if (IsFreeTurnEnabled)
        {
            ...
            if (!PlayerPrefs.HasKey(LAST_FREE_TURN_TIME_NAME)) PlayerPrefs.SetString(LAST_FREE_TURN_TIME_NAME, DateTime.Now.Ticks.ToString());
        }
        else
        {
    ...
    private void TurnWheel(bool isFree)
    {
        ...
        // All sectors angles
        var sectorsAngles = new int[Sectors.Length];
        ...
        for (var i = 1; i <= Sectors.Length; i++) sectorsAngles[i - 1] = 360 / Sectors.Length * i;

        //int cumulativeProbability = Sectors.Sum(sector => sector.Probability);

        double rndNumber = Random.Range(1, Sectors.Sum(sector => sector.Probability));
        ...
        var randomFinalAngle = sectorsAngles[0];
    ...
    public void TurnWheelButtonClick()
    {
        ...
        else
        {
            ...
            if (IsPaidTurnEnabled) // If player have enough coins
    ...
    public bool FreeTurnAvailable()
    {
        return _isFreeTurnAvailable;
    }
 ...
 */
[Serializable]
public class FortuneWheelSector : object
{
...
public class FortuneWheelManagerEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        var myScript = (FortuneWheelManager) target;
        if (GUILayout.Button("Reset Timer")) myScript.ResetTimer();
    }
}
#endif