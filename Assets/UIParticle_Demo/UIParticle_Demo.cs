...
namespace Coffee.UIExtensions.Demo
{
    public class UIParticle_Demo : MonoBehaviour
    {
        ...
        [SerializeField] private List<Transform> m_ScalingByTransforms;
        [SerializeField] private List<UIParticle> m_ScalingByUIParticles;
        ...
        {
            foreach (var m in FindObjectsOfType<Mask>()) m.enabled = enabled;
        }

        public void EnableMask2D(bool enabled)
        {
            foreach (var m in FindObjectsOfType<RectMask2D>()) m.enabled = enabled;
        }
        ...
        {
            foreach (var uip in FindObjectsOfType<UIParticle>()) uip.scale = scale;
        }
        ...
        {
            foreach (var animator in FindObjectsOfType<Animator>()) animator.Play("Play");

            foreach (var particle in FindObjectsOfType<ParticleSystem>()) particle.Play();
        }
        ...
        {
            if (flag) GetComponent<Canvas>().renderMode = RenderMode.ScreenSpaceCamera;
        }

        public void SetOverlay(bool flag)
        {
            if (flag) GetComponent<Canvas>().renderMode = RenderMode.ScreenSpaceOverlay;
        }
    }
}