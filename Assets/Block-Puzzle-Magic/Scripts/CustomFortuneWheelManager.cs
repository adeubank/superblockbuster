...
public class CustomFortuneWheelManager : Singleton<CustomFortuneWheelManager>
{
    ...
    [Header("Params for each sector")] public CustomFortuneWheelSector[] Sectors; // All sectors objects
    ...
    public bool isFreeTurnAvailable;

    private CustomFortuneWheelSector _finalSector;
    ...
    public void ShowFortuneWheel()
    {
        ...
        var fortuneWheelShowSequence = DOTween.Sequence();
        fortuneWheelShowSequence.Append(rootObject.transform.DOScale(Vector3.one * 1f, 0.4f));
        fortuneWheelShowSequence.Append(rootObject.transform.DOScale(Vector3.one * 0.8f, 0.1f));
        fortuneWheelShowSequence.Append(rootObject.transform.DOScale(Vector3.one * 1f, 0.1f));
        fortuneWheelShowSequence.Append(rootObject.transform.DOScale(Vector3.one * 0.9f, 0.1f));
    ...
    public void HideFortuneWheel()
    {
        ...
        rootObject.transform.position = new Vector3(-1200, 0);
    }


    private void ShowTurnButtons()
    {
        if (isFreeTurnAvailable) // If have free turn
        {
            ShowFreeTurnButton();
    ...
    public void RewardCoins(int sectorIndex)
    {
        if (_finalSector == null)
        {
            Debug.Log("Cannot reward coins without final sector!");
            return;
        }
        
        var rewardValue = _finalSector.RewardValue;
        rewardImage.sprite =  _finalSector.RewardSprite;
        rewardText.text =  _finalSector.RewardText;
        ...
        ShowRewardAcceptPowerup(delegate
        {
            ...
            CurrencyManager.Instance.AddCoinBalance(rewardValue);
        });
    }

    /// <summary>
    /// Awards user a powerup
    /// </summary>
    /// <param name="powerupId">Powerup ID for user</param>
    public void RewardPowerup(int sectorIndex)
    {
        if (_finalSector == null)
        {
            Debug.Log("Cannot reward powerup without final sector!");
            return;
        }
        ...
        ShowRewardAcceptPowerup(delegate
        {
            ...
            AudioManager.Instance.PlaySound(CurrencyManager.Instance.coinGainSound);
            PowerupController.Instance.AddPurchasedPowerupId(rewardValue);
        });
    }

    private void ShowRewardAcceptPowerup(Action rewardAction)
    {
        _rewardAction = rewardAction;
        InputManager.Instance.DisableTouch();
        var fortuneWheelShowSequence = DOTween.Sequence();
        fortuneWheelShowSequence.AppendInterval(1f);
        fortuneWheelShowSequence.AppendCallback(() =>
        {
            rewardRootObject.SetActive(true);
            spinnerRootObject.SetActive(false);
        });
        fortuneWheelShowSequence.Append(rewardRootObject.transform.DOScale(Vector3.one * 1f, 0.4f));
        fortuneWheelShowSequence.Append(rewardRootObject.transform.DOScale(Vector3.one * 0.8f, 0.1f));
        fortuneWheelShowSequence.Append(rewardRootObject.transform.DOScale(Vector3.one * 1f, 0.1f));
        fortuneWheelShowSequence.Append(rewardRootObject.transform.DOScale(Vector3.one * 0.9f, 0.1f));
        fortuneWheelShowSequence.AppendCallback(() => { InputManager.Instance.EnableTouch(); });
    }

    public void HideRewardAcceptPowerup()
    {
        if (_rewardAction == null)
        {
            Debug.Log("Cannot reward without a reward action");
        }
        else
        {
            _rewardAction();
 ...
 */
[Serializable]
public class CustomFortuneWheelSector : System.Object
{
    [Tooltip("Sprite used on reward accept screen")]
    public Sprite RewardSprite;

    [Tooltip("Text used on reward accept screen")]
 ...
 */
#if UNITY_EDITOR
[CustomEditor(typeof(CustomFortuneWheelManager))]
public class CustomFortuneWheelManagerEditor : Editor
{
    public override void OnInspectorGUI()
    {
        DrawDefaultInspector();

        CustomFortuneWheelManager myScript = (CustomFortuneWheelManager) target;
